...
object Timer extends Logging {
  ...
  /** Time a synchronous task and log the results.
    *
    * @param taskName
    *   the name of the task, which is added to the written log
    * @param additionalMarkers
    *   any additional markers to add to the log entry
    * @param slowLogThresholdMs
    *   pass to log at `warn` level when the operation exceeds the threshold.
    * @param onSlowLog
    *   evaluated when the operation exceeds the slow log threshold
    */
  def time[R](
      taskName: String,
      additionalMarkers: LogstashMarker = Markers.empty(),
      slowLogThresholdMs: Long = Long.MaxValue,
      onSlowLog: Long => Unit = identity _
  )(block: => R): R = {
  ...
  private def logTime(
      ...
      fromInNs: Long,
      toInNs: Long,
      additionalMarkers: LogstashMarker,
      slowLogThresholdMs: Long,
      onSlowLog: Long => Unit
  ) = {
