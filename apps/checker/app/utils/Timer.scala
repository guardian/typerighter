...
object Timer extends Logging {
  /**
    ...
    */
  def time[R](taskName: String, additionalMarkers: LogstashMarker = Markers.empty(), slowLogThresholdMs: Long = Long.MaxValue, onSlowLog: Long => Unit = identity _)(block: => R): R = {
  ...
  /**
    ...
    * @param onSlowLog evaluated when the operation exceeds the slow log threshold
    */
  def timeAsync[R](taskName: String, additionalMarkers: LogstashMarker = Markers.empty(), slowLogThresholdMs: Long = Long.MaxValue, onSlowLog: Long => Unit = identity _)(block: => Future[R])(implicit ec: ExecutionContext): Future[R] = {
    val t0 = System.nanoTime()
    block.map { result =>
      val t1 = System.nanoTime()
      logTime(taskName, t0, t1, additionalMarkers, slowLogThresholdMs, onSlowLog)
