...
class SpellDictionaryBuilder extends DictionaryBuilder {
  ...
  @throws[Exception]
  private def build(plainTextDictFile: File) =  { var tempFile: File = null
    try {tempFile = tokenizeInput(plainTextDictFile)
      buildFSA(tempFile)} finally if (tempFile != null) tempFile.delete
  }

  def buildDictionary(wordList: List[String]): File = {
    val outputFileName = s"${getClass.getResource("/dictionary").getPath}/collins.dict"
    ...
    try {
      ...
      bw.close()
      val frequencyURL = getClass.getResource("/dictionary/en_gb_wordlist.xml")
  ...
  private def tokenizeInput(plainTextDictFile: File) =  {
    ...
    try {
      ...
      try while (scanner.hasNextLine)  { val line = scanner.nextLine
        val sepPos = if (separator.isEmpty) -1
        else line.indexOf(separator)
        val occurrences = if (sepPos != -1) line.substring(sepPos + separator.length)
        else ""
        val lineWithoutOcc = if (sepPos != -1) line.substring(0, sepPos)
        else line
        val tokens = util.Arrays.asList(lineWithoutOcc)
        tokens.forEach(token =>  {
          if (token.nonEmpty)  {
            out.write(token)
            if (sepPos != -1)  { out.write(separator)
