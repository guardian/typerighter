...
class RulesSpec extends FixtureAnyFlatSpec with Matchers with AutoRollback with DBTest {
  ...
  override def fixture(implicit session: DBSession) = {
    ...
    sql"insert into rules (rule_type, pattern, replacement, category, tags, description, ignore, notes, external_id, force_red_rule, advisory_rule, created_by, updated_by) values (${"regex"}, ${"pattern"}, ${"replacement"}, ${"category"}, ${"someTags"}, ${"description"}, false, ${"notes"}, ${"externalId"}, false, false, 'test.user', 'test.user')"
      .update()
      .apply()
  }
