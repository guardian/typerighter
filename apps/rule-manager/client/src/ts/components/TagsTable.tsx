...
const DeleteTag = ({
	...
	>
		<DeleteTagButton
			...
			onClick={() => (editIsEnabled ? openDeleteTagDialogue(tag) : null)}
		>
			{isLoading && tagToDelete && tagToDelete.id === tag.id ? (
				<EuiLoadingSpinner />
			) : (
	...
	return [
		{
			...
			render: (tagName, item) => (
				<EditableNameField
					editIsEnabled={hasEditPermissions}
					editTag={editTag}
					tag={item}
					fetchTags={fetchTags}
					key={item.id}
			...
			actions: [
				{
					...
					render: (item, enabled) => (
						<DeleteTag
							editIsEnabled={enabled}
							tag={item}
							openDeleteTagDialogue={openDeleteTagDialogue}
							isLoading={isLoading}
...
const CreateTagForm = ({
	...
	const [clientSideValidationError, setClientSideValidationError] = useState<
		string | null
	>(null);
			...
			<RuleFormSection title="CREATE NEW TAG">
				...
				>
					<EuiFlexItem grow={true}>
						...
						>
							<EuiFieldText
								...
								value={tagName}
								onChange={(e) => {
									setClientSideValidationError(null);
					...
					<EuiFlexItem grow={false}>
						...
						>
							<EuiButton
								onClick={() => createTagIfSuitable(tagName)}
								color={'primary'}
								fill
								disabled={!enabled || isLoadingCreatedTag}
							>
								{isLoadingCreatedTag ? (
...
const DeleteTagWarning = ({
	...
	setTagToDelete,
	deleteTag,
	setHideDeletionModal,
}: {
	tag: Tag;
	setTagToDelete: (tag: Tag | null) => void;
	deleteTag: (tag: Tag) => Promise<ErrorIResponse | undefined>;
	setHideDeletionModal: (bool: boolean) => void;
}) => {
		...
		>
			...
			<EuiModalBody>
				<EuiText>
					...
					<br />
					This action is <strong>permanent</strong> and will remove the tag from
				...
				<EuiFlexGroup justifyContent="flexEnd">
					...
					<EuiFlexItem grow={false}>
						<EuiButton
							onClick={() => deleteAndClosePrompty(tag)}
							color="danger"
						>
...
export const TagsTable = () => {
	...
	const columns = createTagTableColumns(
		updateTag,
		fetchTags,
		isLoading,
		openDeleteTagDialogue,
					...
					<EuiFlexGroup style={{ overflowY: 'scroll', height: '100%' }}>
						{tagToDelete && !hideDeletionModal ? (
							<DeleteTagWarning
								...
								deleteTag={deleteTag}
								setHideDeletionModal={setHideDeletionModal}
							/>
						) : null}
						<EuiFlexItem>
							<EuiInMemoryTable
