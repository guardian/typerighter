...
export const RuleDataLastUpdated: React.FC<{ ruleData: RuleData, isLoading: boolean, hasErrors: boolean }> = ({
  isLoading,
  ruleData,
  hasErrors
}) => {
  ...
  const formatStr = isLessThanADayOld
    ? "HH:mm:ss"
    : "do LLL yyyy";

  const iconColor = hasErrors ? "danger" : "success";
  const iconName = hasErrors ? "error" : "checkInCircleFilled";
  const text = hasErrors ? "Error saving changes" : `Saved ${format(latestUpdatedAt, formatStr)}`;

  return (
    <EuiFlexGroup gutterSize="s" alignItems="center">
      <span>{text}</span>
      {isLoading ? <EuiLoadingSpinner size="s" /> : <EuiIcon color={iconColor} type={iconName} size="s" />}
    </EuiFlexGroup>
  );
};
