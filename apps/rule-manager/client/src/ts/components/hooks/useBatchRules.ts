...
export function useBatchRules(ruleIds: number[] | undefined) {
	...
	const fetchRules = async (ruleIds: number[]) => {
		...
		try {
			const response = await fetch(
				`${location.origin}/api/rules/batch/${ruleIds.join(',')}`,
			);
			if (!response.ok) {
				throw new Error(
	...
	const updateRules = async (ruleForm: DraftRule[]) => {
		...
		const hasCategoryChanged =
			rules && rules[0].draft.category !== ruleForm[0].category;
		...
		if (parsedResponse.status === 'ok') {
			if (rules && parsedResponse.data) {
				const updatedRules = rules.map((rule, index) => {
					const updatedRule = (
						parsedResponse.data as unknown as DraftRule[]
					).find((updatedRule) => rule.draft.id === updatedRule.id);
