import {
	FieldObject,
	findFieldsWithDiffs,
	findNonIntersectingFields,
	transformToHumanReadableValues,
} from './Diff';
...
describe('Diff helper functions', () => {
	describe('findFieldsWithDiffs', () => {
		...
		it('should not provide diffs for identical tag arrays', () => {
			...
			const expectedDiff = expectedDiffFields.filter(
				(field) => field.fieldName != 'tags',
			);
			const fieldsWithDiffs = findFieldsWithDiffs(
				mockRuleDataWithIdenticalTags,
			);
	...
	describe('getHumanReadableValues', () => {
		it('should convert ruleTypes to the names used on the rule form', () => {
			const humanReadableFields = transformToHumanReadableValues(
				expectedDiffFields,
				mockTags,
			);
			const languageToolName = ruleTypeOptions.find(
				(option) => option.id === 'languageToolXML',
			)?.label;
			const regexName = ruleTypeOptions.find(
				(option) => option.id === 'regex',
			)?.label;
			const expectedRuleTypeField = {
				fieldName: 'ruleType',
				draft: regexName,
				live: languageToolName,
			};
			expect(
				humanReadableFields.find((field) => field.fieldName === 'ruleType'),
			).toEqual(expectedRuleTypeField);
		});
		it('should convert tags from their IDs to their readable names', () => {
			...
			expect(
				humanReadableFields.find((field) => field.fieldName === 'tags'),
			).toEqual(expectedTagsField);
		});
	});
