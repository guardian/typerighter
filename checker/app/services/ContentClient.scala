...
class ContentClient(client: GuardianContentClient) {
  ...
  def searchContent(
    ...
    val queryWithSections = sections.foldLeft(queryWithTags) {
      case (q, section) => q.section(section)
    }
  ...
  def searchTags(
      tags: String
  )(implicit ec: ExecutionContext): Future[TagsResponse] = {
  ...
  def searchSections(
      sections: String
  )(implicit ec: ExecutionContext): Future[SectionsResponse] = {
