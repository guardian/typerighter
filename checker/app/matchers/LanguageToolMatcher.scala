...
class LanguageToolFactory(
  ...
  def createInstance(
      ruleXMLs: List[LTRuleXML],
      defaultRuleIds: List[String] = Nil
  )(implicit ec: ExecutionContext): Either[List[Throwable], Matcher] = {
    ...
    maybeLanguageModelDir.foreach { languageModel =>
      ...
      if (useLanguageModelRules)
        instance.activateLanguageModelRules(languageModel)
    }
  ...
  private def getLTRulesFromXML(
      rules: List[LTRuleXML]
  ): Try[List[AbstractPatternRule]] = rules match {
      ...
      getXMLStreamFromLTRules(rules) flatMap { xmlStream =>
        {
          Try(
            loader
              .getRules(xmlStream, "languagetool-generated-xml")
              .asScala
              .toList
          )
        }
  ...
  /** Attempt to get an XML stream representing a valid XML document containing the given rules.
    ...
    */
  private def getXMLStreamFromLTRules(
      rules: List[LTRuleXML]
  ): Try[ByteArrayInputStream] = Try {
    ...
    val rulesXml = rulesByCategory.map { case (category, rules) =>
      <category id={category.id} name={category.name} type="grammar">
        ...
        rules.map { rule =>
          try {
            XML.loadString(rule.xml) % Attribute(
              None,
              "id",
              Text(rule.id),
              Null
            ) % Attribute(None, "name", Text(rule.description), Null)
          } catch {
            case e: Throwable =>
  ...
  private def annotateExceptionWithRuleId(
    ...
    new Exception(
      s"Error applying LanguageTool rule `${ruleId}`: ${e.getMessage} ${if (additionalMessage.size > 0) additionalMessage
      else ""}".trim,
      e
    )
}
...
class LanguageToolMatcher(instance: JLanguageTool) extends Matcher {
  ...
  )(implicit ec: ExecutionContext): Future[List[RuleMatch]] = Future {
    request.blocks.flatMap { block =>
      instance
        .check(block.text)
        .asScala
        .map(RuleMatch.fromLT(_, block, getType))
        .toList
        .map { ruleMatch =>
          ruleMatch.copy(
