...
class AppComponents(
  ...
  private val logShipping = Some(identity).collect {
    case awsIdentity: AwsIdentity =>
      val loggingStreamName =
        configuration.getOptional[String]("typerighter.loggingStreamName")
      new ElkLogging(
        awsIdentity,
        loggingStreamName,
        creds,
  ...
  val ngramPath: Option[File] =
    configuration.getOptional[String]("typerighter.ngramPath").map(new File(_))
  ...
  val settingsFile = identity match {
    ...
    case identity: AwsIdentity =>
      s"${identity.stage.toLowerCase}.dev-gutools.co.uk.settings.public"
    case _: DevIdentity => "local.dev-gutools.co.uk.settings.public"
  }
  val publicSettings =
    new PublicSettings(settingsFile, "pan-domain-auth-settings", s3Client)
  ...
  val matcherPoolDispatcher =
    actorSystem.dispatchers.lookup("matcher-pool-dispatcher")
  ...
  val sheetsRuleManager = new SheetsRuleManager(
    credentials,
    spreadsheetId,
    matcherPool,
    languageToolFactory
  ...
  val apiController =
    new ApiController(controllerComponents, matcherPool, publicSettings)
  ...
  val auditController =
    new AuditController(controllerComponents, publicSettings)
