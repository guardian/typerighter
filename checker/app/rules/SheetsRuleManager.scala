...
class SheetsRuleManager(
  ...
  def getRules()(implicit
      ec: ExecutionContext
  ): Either[List[String], RuleResource] = {
    ...
    (maybeRules, maybeLTRuleIds) match {
      ...
      case (mamaybeRules, maybeLt) =>
        Left(
          maybeLTRuleIds.left.getOrElse(Nil) ++ maybeRules.left.getOrElse(Nil)
        )
    }
  }
  ...
  private def getRuleFromRow(
    ...
    try {
      ...
      (maybeId, maybeIgnore, ruleType) match {
        ...
        case (None, _, _) =>
          Failure(new Exception(s"no id for rule (row: ${rowNumber})"))
          ...
          Success(
            Some(
              getRegexRule(
                ...
                row
                  .lift(PatternRuleCols.Description)
        ...
        case (Some(id), _, ruleType) =>
          Failure(
            new Exception(
              s"Rule type ${ruleType} for rule with id ${id} not supported"
            )
          )
      }
      ...
      case e: Throwable => {
        Failure(
          new Exception(
            s"Error parsing rule at index ${index} – ${e.getMessage()}"
          )
        )
      }
    }
  ...
  private def getRegexRule(
    ...
    replacement =
      if (suggestion.isEmpty) None else Some(TextSuggestion(suggestion)),
    regex = pattern.r
  )
