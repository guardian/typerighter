...
object Text {
  def getSurroundingText(text: String, from: Int, to: Int): (String, String) = {
    val buffer = 100
    val precedingText =
      text.substring(scala.math.max(from - buffer, 0), scala.math.max(from, 0))
    val subsequentText = text.substring(
      scala.math.min(to, text.length),
      scala.math.min(to + buffer, text.length)
    )
