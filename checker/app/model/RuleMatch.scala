...
object RuleMatch {
  def fromLT(
    ...
    val replacement =
      if (suggestions.size == 1) Some(suggestions.head) else None
...
case class RuleMatch(
  ...
  /** Map the range this match applies to through the given ranges, adjusting its range accordingly.
    */
  def mapThroughSkippedRanges(skipRanges: List[TextRange]): RuleMatch =
    skipRanges match {
      case Nil => this
      case skipRanges => {
        val (newMatch, _) =
          skipRanges.foldLeft((this, List.empty[TextRange]))((acc, range) =>
            acc match {
              case (ruleMatch, rangesAlreadyApplied) => {
                val newMatchRange =
                  TextRange(ruleMatch.fromPos, ruleMatch.toPos)
                    .mapAddedRange(range)
                val newRuleMatch = ruleMatch.copy(
                  fromPos = newMatchRange.from,
                  toPos = newMatchRange.to
                )
  ...
  def mapMatchThroughBlocks(blocks: List[TextBlock]): RuleMatch = {
    val maybeBlockForThisMatch =
      blocks.find(block => fromPos >= block.from && toPos <= block.to)
    val skipRangesForThisBlock =
      maybeBlockForThisMatch.flatMap(_.skipRanges).getOrElse(Nil)
