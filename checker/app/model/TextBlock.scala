      ...
      .foldLeft((this, List.empty[TextRange]))((acc, range) =>
        acc match {
          case (block, rangesAlreadyApplied) => {
            val mappedRange = rangesAlreadyApplied
              .foldRight(range)((acc, range) => range.mapRemovedRange(acc))
            val snipFrom = mappedRange.from - block.from
            val snipTo = snipFrom + (mappedRange.to - mappedRange.from)
            val snipRange =
              TextRange(Math.max(snipFrom, 0), Math.min(block.to, snipTo + 1))

            val newText = block.text.slice(0, snipRange.from) + block.text
              .slice(snipRange.to, block.text.size)
            val newBlock = block.copy(
              text = newText,
              to = block.from + newText.length,
              skipRanges = None
            )
